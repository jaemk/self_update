...
|------|------|-------|-------|
| 0 — mechanical | All GitHub API ops, CI, README regen, push preamble | 1, 5, 6, 8(preamble), 9, 10 | script (`pr.py`) |
...
- **PR number**: if omitted, infer it from the current branch using `gh pr view --json number`. In `local` mode the PR number is only needed for the `pr.py` wrapper commands (`ci`, `readme`, `pushpreview`) and for the push preamble — none of which read or mutate the PR conversation.
...
`threads` lists all review thread node IDs with their resolution status. Unresolved threads will all be resolved in step 9.
...
- **Invalid**: the finding is incorrect or environment-specific
...
Test design always stays here — a fix that changes behavior needs a test that fails
on the unfixed code and passes on the fixed code; a trivially-passing test defeats the
rule.

Common fix types and where they live:
- Library/backend logic: `src/update.rs`, `src/backends/{github,gitlab,gitea,s3}.rs`, `src/backends/common.rs` (shared `CommonConfig`), `src/lib.rs`, `src/macros.rs`, `src/http_client/`
- Tests: in-module `#[cfg(test)] mod tests` blocks (e.g. in `src/update.rs`, `src/backends/s3.rs`) and doctests in `src/lib.rs`
- Docs that feed the README: `src/lib.rs` crate-level doc comment (never edit `README.md` directly)
...
- **Delegate to `pr-fix-implementer` (Sonnet)** when the fix is **mechanical or
  repetitive across multiple files** (e.g. the same change applied to all four
  backends, a doc-string pattern replicated across modules). State "delegating
  because: <reason>". Spawn the `pr-fix-implementer` agent with the fix spec as the
  prompt.
...
This runs the project checks — `cargo fmt --check`, `clippy` and `test` on **both** the
`reqwest` and `ureq` clients across the full feature set, and a README drift check — and
exits non-zero only on real failures. If it exits non-zero, fix the reported failures
and re-run.
...
No-ops automatically when `src/lib.rs` is unchanged vs `origin/master`. `README.md` is
generated from the `src/lib.rs` crate doc — never edit it by hand.
...
- Check accuracy of any named types, method signatures, builder/setter names, or feature gates — they must exactly match the code.
...
- Feature/behavior claims that no longer apply
...
github pagination fix and CHANGELOG update"). Then push:

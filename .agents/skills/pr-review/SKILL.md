...
**Does NOT:** edit files, run the CI checks, regenerate the README, commit, or push;
and it does **not** interact with the GitHub PR conversation — it does not read
existing PR comments/threads, resolve or minimize them, edit the PR body, or
re-request Copilot review. Those belong to `pr-cycle`. This skill only generates a
fresh agent-based review of the code itself.
...
- **Target**: either a **PR number**, or **nothing** (review the current checked-out
  ...
  `gh pr view --json number` (run with the sandbox disabled — GitHub needs network),
  but a PR is **not required**: a plain checked-out branch is reviewed by diffing
...
the current branch with **opus** reviewers" or "Reviewing PR #181 with Sonnet
...
- **Invalid** — the finding is incorrect or environment-specific (e.g. a rustc-version
  diagnostic difference, or a "missing" feature gate that is actually present).

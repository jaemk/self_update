//! End-to-end check of `auth_token_from_env()` on the **gitlab** builders against a real process
...
/// One test, one process: pins the whole env-token contract on the two gitlab builders against a
/// genuinely populated environment, observed where it actually matters -- the `Authorization`
/// header on the wire.
///
/// The in-crate gitlab tests run in whatever environment cargo was invoked with, which is normally
/// clean, so their "an explicit token wins over the environment" assertions hold **vacuously**:
/// nothing was resolved to lose to. This file exports `GITLAB_TOKEN` first, so the explicit token
...
/// application's own). It also pins gitlab's `Bearer` scheme for an env-sourced token -- a token
/// resolved from the environment must be rendered exactly like an explicit one.
#[test]
fn env_token_reaches_the_wire_and_an_explicit_token_still_wins() {
    ...
    //    and rendered with gitlab's `Bearer` scheme.
    let mut list = gitlab::ReleaseList::configure();
    ...
    assert!(
        list.has_auth_token(),
        "auth_token_from_env() must pick up GITLAB_TOKEN from the process environment"
    );
    ...
    assert_eq!(
        ...
        "the env-resolved token must reach the wire with gitlab's Bearer scheme"
    );

    // 2. Pickup on the Update builder.
    let mut upd = gitlab::Update::configure();
    upd.repo_owner("o")
        .repo_name("r")
        .bin_name("app")

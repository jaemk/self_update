...
`get_latest_release()` and `get_newer_releases()` return `Result<Releases>` (the
latter no longer takes a `current_version` argument, and was renamed from
`get_latest_releases`), while `get_release_version(ver)` is unchanged. The per-updater
`is_update_available` / `is_update_available_async` checks were removed; the pre-check
is now one fetch, `updater.get_newer_releases()?.is_update_available()`.
...
`get_latest_release_async()` and `get_newer_releases_async()` returning
...
the current version). `get_newer_releases()` returns the strictly-newer-filtered
list. Code wanting "is there a newer release" should use `get_newer_releases()`.

See `src/update.rs:Releases`, the crate-root re-export in `src/lib.rs`, and the
CHANGELOG `[1.0.0]` Added and Changed entries.

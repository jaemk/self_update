...
/// with [`Release::from_release_gitlab`] and following `Link: rel="next"`. See github's
...
fn release_array_page(
    ...
    PageRequest {
        ...
        parse: Box::new(move |body, resp_headers| {
            ...
            let mut stop = false;
            for value in array {
                let release = Release::from_release_gitlab(value)?;
...
fn single_plan(url: String, auth_token: Option<&str>) -> Result<PageRequest<Release>> {
    ...
    Ok(PageRequest {
        ...
        parse: Box::new(|body, _resp_headers| {
            let json: serde_json::Value = serde_json::from_slice(body)?;
            Ok(Page::last(vec![Release::from_release_gitlab(&json)?]))
        }),
    })
}
